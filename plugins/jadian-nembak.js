...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `Masukan Nomor atau Tag doi, bisa juga Reply pesan doi`, m)
  
  if(isNaN(number)) return conn.reply(m.chat, `_*Nomor tidak valid.*_`, m)
  if(number.length > 15) return conn.reply(m.chat, `_*Format tidak valid.*_`, m)
  try {
		if(text) {
    ...
    if(!users) return conn.reply(m.chat, `*Nomor doi tidak ada di grup ini. Mungkin sudah keluar*_`, m)
    if(user === m.sender) return conn.reply(m.chat, `_*Kamu gabisa pacaran sama diri sendiri kak*_`, m)
    if(user === conn.user.jid) return conn.reply(m.chat, `_*Kamu gabisa nembak UreshiiBotz, karena saya sepenuhnya milik Farhan 😋*_`, m)

    if (typeof global.db.data.users[user] == "undefined") return m.reply("_*Orang yang kamu tag tidak ada di database. Ajak dia Registrasi atau Daftar dulu*_")
    ...
    if(global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user){
      conn.reply(m.chat, `Kak?. Kamu udah pacaran lho sama @${global.db.data.users[m.sender].pasangan.split('@')[0]}\n\nSilahkan putus terlebih dahulu dari @${user.split('@')[0]}\n\nBtw kakak harus lebih setia lagi ya!. Masa udah ada pasangan masi nembak orang 🗿`, m , { contextInfo: { mentionedJid: [user, global.db.data.users[m.sender].pasangan]}})
      ...
      if (pacar){
        ...
        conn.reply(m.chat, `Maaf kak, @${user.split('@')[0]} Sudah berpacaran dengan @${pacar.split('@')[0]}\nSilahkan cari pasangan lain!\n\nNt ya kak! :)`, m , { contextInfo: { mentionedJid: [user, pacar]}})
      }else{
        global.db.data.users[m.sender].pasangan = user
        conn.reply(m.chat, `${await ktnmbk.getRandom()}\n\nKamu baru saja mengajak @${user.split('@')[0]} untuk berpacaran\n\nSilahkan menunggu jawaban darinya!\n\nKetik *${usedPrefix}terima @user* untuk menerima\n*${usedPrefix}tolak @user* untuk menolak`, m , { contextInfo: { mentionedJid: [user]}})
      }
    }else if (global.db.data.users[user].pasangan == m.sender){
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `Cieee, selamat kak!. Kamu di terima oleh @${user.split('@')[0]}\n\nSemoga langgeng dan bahagia selalu ya kak 😋`, m , { contextInfo: { mentionedJid: [user]}})
    }else {
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `${await ktnmbk.getRandom()}\n\nKamu baru saja mengajak @${user.split('@')[0]} berpacaran\n\nSilahkan menunggu jawaban darinya!\n\nUntuk kak @${user.split('@')[0]} silahkan ketik *${usedPrefix}terima @user* untuk menerima ajakannya\n*${usedPrefix}tolak @user* untuk menolak`, m , { contextInfo: { mentionedJid: [user]}})
    }
	}	
}
handler.help = ['tembak']
