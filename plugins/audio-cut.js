...
let handler = async (m, { conn, args, usedPrefix, command }) => {
        ...
        if (!/audio/.test(mime)) throw `Reply Vn/Audio dengan caption *${usedPrefix + command}*`
        let audio = await q.download?.()
        if (!audio) throw 'Yah Gagal'
        if (!args[0] || !args[1]) throw `Format salag\nContoh: ${usedPrefix + command} 00:00:30 00:00:30`
            ...
            exec(`ffmpeg -ss ${args[0]} -i ${media} -t ${args[1]} -c copy ${filename}`, async (err) => {
                ...
                if (err) return Promise.reject( `Yah Gagal`)
