...
export async function before(m, { conn, isAdmin, isBotAdmin }) {
    ...
    if (chat.antiVirtex && isAntiVirtex && m.isGroup) {
        if (isBotAdmin && !isAdmin) {
            await conn.reply(m.chat, 'Ups, kamu terdeteksi mengirimkan;*Virtex*. Maka kamu akan saya kick', false)
            conn.sendMessage(m.chat, { delete: m.key })
        } else {
            conn.reply(m.chat, 'Ups, bot bukan admin. Jadi bot tidak bisa kick!')
        }
    }
    return
}