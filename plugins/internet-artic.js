...
var salah_input = "Format salah\nContoh: .artic Cyberpunk"
if (!text) throw salah_input
await m.reply(wait)
try {
    let arti = await searchArtworks(text)
    let arrd = arti[Math.floor(Math.random() * arti.length)]
    await conn.sendFile(m.chat, "https://www.artic.edu/iiif/2/" + arrd.image + "/full/843,/0/default.jpg", arrd.id, "☁️ *Result:*\n" + arrd.title, m)
    } catch (e) {
    throw eror
    }
}
