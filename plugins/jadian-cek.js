let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  if (global.db.data.users[user].pasangan == "") {
    conn.reply(m.chat, `${orang} Tidak memiliki pasangan dan sedang tidak menembak siapapun 😋\nKetik *.tembak @user* Untuk menembak seseorang!`, m)
  }else if (global.db.data.users[global.db.data.users[user].pasangan].pasangan != user){
    conn.reply(m.chat, `${orang} Sedang menunggu jawaban yang tidak pasti dari @${global.db.data.users[user].pasangan.split('@')[0]} karena sedang tidak diterima atau di tolak\nKetik *${usedPrefix}ikhlasin* untuk mengikhlaskan!`, m,{contextInfo: {
